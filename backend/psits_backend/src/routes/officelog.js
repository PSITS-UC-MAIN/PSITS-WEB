    ...
    try {
      ...
      if (foundLog)
        return res.status(403).json({
          foundLog,
          message: "Cannot override found log, you must log out first",
          StatusCode: 403,
        });
      // save
      ...
      res.status(201).json({
        savedOfficeLog,
        message: "Office log was recorded",
        StatusCode: 201,
      });
    } catch (error) {
      res.status(400).json({
        message: "Error on saving office log info : " + error.message,
        StatusCode: 400,
      });
    }
  }
...
router.patch(
  ...
  async (req, res) => {
    ...
    const min =
      req.headers.minval ??
      new Date(new Date().setDate(new Date().getDate() - 100)).toISOString();
    try {
      const foundLog = await OfficeLogModel.findOne({
        user: res.issuer,
        loginTime: { $gte: min, $lt: max },
      ...
      if (!foundLog)
        return res.status(404).json({
          message: "No pending office log found, you have to log in first",
          StatusCode: 404,
        });
      ...
      res.status(200).json({
        updatedOfficeLog,
        message: "Office log was updated",
        StatusCode: 200,
      });
    } catch (error) {
      res.status(400).json({
