...
import { ParseAnnouncement } from "../utils/ServerUtils.js";
...
routes.post("/", GetAuthToken, VerifyAdmin, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      NewAnnouncement: await ParseAnnouncement(announcement),
      message: "Posted announcement",
      StatusCode: 200,
    });
  } catch (error) {
    res.status(400).json({
      message: "Failed to post announcement | error : " + error.message,
      StatusCode: 400,
    });
  }
});
  ...
  async (req, res) => {
    if (!req.params.announcementid)
      return res.status(400).json({
        message: "AnnouncementID must be a query param",
        StatusCode: 400,
      });
    ...
    try {
      ...
      if (!announcement)
        return res
          .status(404)
          .json({ message: "Announcement not found", StatusCode: 404 });
      ...
      try {
        ...
        res.json({
          NewAnnouncement: await ParseAnnouncement(newAnnouncement),
          message: "Updated Announcement",
          StatusCode: 200,
        });
      } catch (e) {
        res.status(400).json({
          message: "Failed to update announcement | error: " + e.message,
          StatusCode: 400,
        });
      }
    } catch (e) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        res.status(400).json({
          message: "Failed to delete announcement | error: " + e.message,
          StatusCode: 400,
        });
      }
    } catch (e) {
