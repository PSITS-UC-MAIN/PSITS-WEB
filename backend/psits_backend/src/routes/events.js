...
routes.post("/", GetAuthToken, VerifyAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ newEvent, message: "Event was posted", StatusCode: 201 });
  } catch (e) {
    res.status(400).json({
      message: "Failed to post event | error : " + e.message,
      StatusCode: 400,
    });
  }
});
...
routes.patch("/:eventid", GetAuthToken, VerifyAdmin, async (req, res) => {
  if (!req.params.eventid)
    return res.status(400).json({
      message: "EventID must be provided at the request parameter",
      StatusCode: 400,
    });
  ...
  try {
    ...
    if (!event)
      return res
        .status(404)
        .json({ message: "Cannot find School Event", StatusCode: 404 });

    if (req.body.title) event.title = req.body.title;
    if (req.body.eventDate) event.eventDate = req.body.eventDate;
    if (req.body.content) event.content = req.body.content;
    if (req.photo_img_links) event.photo_img_links = req.body.photo_img_links;
    ...
    try {
      ...
      res.status(400).json({
        message: "Failed to update School Events | error: " + e.message,
        StatusCode: 400,
      });
    }
  } catch (e) {
