...
routes.post("/", GetAuthToken, async (req, res) => {
  ...
  try {
    ...
    if (!merch)
      return res
        .status(404)
        .json({ message: "Merchandise was not found", StatusCode: 404 });

    const pendingOrder = await UserOrders.findOne({
      student_id: res.issuerObjectId,
      merch_id: merch._id,
    });
    ...
    if (pendingOrder) {
      ...
      if (
        pendingOrder.status === OrderStatusCode.ORDERED ||
        pendingOrder.status === OrderStatusCode.PAID
      )
        return res.status(409).json({
          message: `You have a pending order ref: ${pendingOrder.reference} that haven't completed yet`,
          StatusCode: 409,
        });
    }
    ...
    try {
      ...
      if (order.quantity > merchstock)
        return res.status(400).json({
          message: "Order quantity is greater than the merchandise stock",
          StatusCode: 400,
        });
      // save
      ...
      res.status(400).json({
        message: "Failed place order | error: " + e.message,
        StatusCode: 400,
      });
    }
  } catch (e) {
...
routes.get("/:search", GetAuthToken, async (req, res) => {
  ...
  if (!req.params.search)
    return res
      .status(400)
      .json({ message: "Specify the search parameter", StatusCode: 400 });
  ...
  try {
    ...
    if (!order)
      return res.status(404).json({
        message: "Failed to retrieved order, not found",
        StatusCode: 404,
      });
    ...
    if (
      !res.isAdmin ||
      res.issuerObjectId.toString() !== order.student_id.toString()
    )
      return res.status(403).json({
        message: "You are forbidden to access this order information",
        StatusCode: 403,
      });
    ...
    res.status(400).json({
      message:
        "Failed to retrieve order info based on the search parameter provided",
      StatusCode: 400,
    });
  }
});
...
routes.patch("/:search", GetAuthToken, async (req, res) => {
  ...
  try {
    ...
    const { information, size, color, review, rating, quantity, style } =
      req.body;
    ...
    if (old_status === OrderStatusCode.CANCELLED && !res.isAdmin)
      return res.status(403).json({
        message: "You are forbidden to update already cancelled order!",
        StatusCode: 403,
      });

    // update basic order info
    if (information) order.information = information;
    ...
    if (status !== OrderStatusCode.INVALID) {
      ...
      if (status === OrderStatusCode.CLAIMED || status === OrderStatusCode.PAID)
        if (!res.isAdmin)
          return res.status(403).json({
            message: "You are forbidden to update the status",
            StatusCode: 403,
          });
      ...
      if (
        ...
        old_status !== OrderStatusCode.CANCELLED
      )
      ...
      if (res.issuerObjectId.toString() === order.student_id.toString()) {
        ...
        if (status === OrderStatusCode.REVIEWED) {
          if (old_status === OrderStatusCode.CLAIMED) {
            ...
            if (rating) {
              merch.rating =
                merch.rating > 0 ? (merch.rating + rating) / 2 : rating;
              order.rating = rating;
            }
            if (review) order.review = review;
          } else {
            return res.status(403).json({
              message: "You are forbidden to review the order",
              StatusCode: 403,
            });
          }
        }
    ...
    res.json({
      updatedOrder,
      message: "Updated Order Information",
      StatusCode: 200,
    });
  } catch (error) {
    // return a bad request when something happens with the parameter value
    res.status(400).json({
      message: "Failed to retrieve order info. error: " + error.message,
      StatusCode: 400,
    });
  }
});
