...
import { SHA512, EncapulateUser } from "../utils/ServerUtils.js";
import Users from "../models/UserModel.js";
import { GetAuthToken, ValidateAPIKey } from "../middlewares/authMiddleware.js";
...
router.post("/", ValidateAPIKey, async (req, res) => {
  ...
  try {
    ...
    const existingUserId = await Users.findOne({ user_id: user.user_id });
    const existingMail = await Users.findOne({ email: user.email });

    if (existingMail || existingUserId)
      return res.status(409).json({
        message: `Failed to created account, data already exists in the database`,
        StatusCode: 409,
      });
    ...
    res
      .status(201)
      .json({ newUser, message: "User account created", StatusCode: 201 });
  } catch (error) {
    res.status(400).json({
      message: "Error on saving user info : " + error.message,
      StatusCode: 400,
    });
  }
});
...
router.get("/:user_id", GetAuthToken, async (req, res) => {
  ...
  if (user_id !== res.issuer) {
    return res.status(403).json({
      message: "User ID does not match with the AuthToken issuer",
      StatusCode: 403,
    });
  }
  ...
  try {
    ...
    if (!user)
      return res.status(404).json({
        message: "Requested user information was not found!",
        StatusCode: 404,
      });

    res.status(200).json({
      UserData: EncapulateUser(user),
...
router.get("/", GetAuthToken, async (req, res) => {
  if (!res.isAdmin)
    return res
      .status(404)
      .json({ message: "AuthToken provided is not allowed", StatusCode: 404 });
...
router.patch("/:user_id", GetAuthToken, async (req, res) => {
  ...
  if (user_id !== res.issuer) {
    if (!res.isAdmin)
      return res.status(403).json({
        message:
          "You are forbidden to update this user account. AuthToken must match with the issuer_id",
        StatusCode: 403,
      });
  }
  try {
    ...
    if (user_id === res.issuer) {
      ...
      if (req.body.password) user.password = await SHA512(req.body.password);
    }
    // update profile img
    ...
    // update graduated
    if (req.body.graduated !== undefined) user.graduated = req.body.graduated;

    // update admin credentials, only if the AuthToken provided has admin rights
    if (res.isAdmin) {
      if ("isAdmin" in req.body) user.isAdmin = req.body.isAdmin;
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ updatedUser, message: "User info Updated", StatusCode: 200 });
    } catch (error) {
      res.status(400).json({
...
router.delete("/:user_id", GetAuthToken, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "User Data was deleted", StatusCode: 200 });
    } catch (error) {
      res.status(500).json({
